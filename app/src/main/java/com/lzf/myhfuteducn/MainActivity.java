...
public class MainActivity extends AppCompatActivity
    ...
    private JSONObject semesterWeekList; //所有学期以及按照学期分的周相关信息
    private String semestercode; //学期代码
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.nav_course) {
            toolbarTitle.setText("课程表");
            fTransaction.replace(R.id.centerContent, new CourseFragment());
                        ...
                        try {
                            ...
                            if (responseJson.getInt("code") == 200) {
                                ...
                                if (objJson != null) {
                                    ...
                                    if (semesterWeekList != null) {
                                        getWeekSchedule(semesterWeekList.getString("cur_semester_code"), semesterWeekList.getString("cur_week_index"));
                                    } else {
                                        Toast.makeText(MainActivity.this, objJson.getString("err_msg"), Toast.LENGTH_SHORT).show();
                                    }
                                }
    ...
    private void addMenu(Menu menu) {
        try {
            ...
            for (int i = 0; i < semesters.length(); i++) {
                menu.add(0, semesters.getJSONObject(i).getInt("code"), 0, semesters.getJSONObject(i).getString("name"));
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
                    ...
                    public void run() {
                        try {
                            ...
                            if (responseJson.getInt("code") == 200) {
                                ...
                                JSONObject objJson = responseJson.getJSONObject("obj");
                                if (objJson != null) {
                                    JSONArray business_data = objJson.getJSONArray("business_data");
                                    if (business_data != null && business_data.length() > 0) {
